...
# 用新数据，新变量；
HTL_raw <- read.csv(file = "D:/project/【HTL】Offline虚拟保留房模型/模型/data/htlsprfestival0218_0224.txt" ,sep ='\t') # 2015.1.1~1.5 新数据结果
# 非平时数据集来进行训练；
# HTL_raw <- read.csv(file = "D:/project/【HTL】Offline虚拟保留房模型/模型/data/htltrain10_15.txt" ,sep ='\t') # 1.6~1.15 新数据结果
HTL <- filter(HTL_raw, d >='2015-02-24' & d <='2015-02-24')
...
HTL$orddays <- ifelse(HTL$orddays_trans >= 3, -1, HTL$orddays_trans)  # 超过3的归结为一类，与其他的category来进行平衡；
                             ...
                             ,ifelse(HTL$ordprior>7 & HTL$ordprior <=14,14
                                     ,ifelse(HTL$ordprior>14 & HTL$ordprior <=21,21
                                             ,ifelse(HTL$ordprior>21 & HTL$ordprior <=30,30,
                                                ifelse(HTL$ordprior >30, 35, HTL$ordprior)
                                             ))))
...
#  ## 提炼出测试集和训练集 ##
set.seed(121)
trainsample <- rbinom(dim(HTL)[[1]], 1, 0.2)==1
...
raw.gbm <- gbm(isfullroom ~
               ...
               +ordhour  # 2 重要变量   leve l: 12.17132199
...
View(tail(resultPred.test2))

# write.csv(HTLtest, file ="D:/project/【HTL】Offline虚拟保留房模型/模型/test/gbm.final.20150312.csv")
