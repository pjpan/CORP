...
# 对比不同模型的结果：
p <- ggplot(data = rocv6.0)+geom_line(aes(x= recall , y=  accuracy))
# p <- p+geom_line(data = rocv6.3,aes(x= recall , y=  accuracy), colour= "red")
p <- p+geom_line(data = rocv5.0,aes(x= recall , y=  accuracy), color= "blue") # 线上的模型
# p <- p+geom_line(data = rocv6.6,aes(x= recall , y=  accuracy), color= "green")
# p <- p+geom_line(data = rocv6.7,aes(x= recall , y=  accuracy), color= "purple")
p+geom_abline(intercept=0.99 , slope=0 ,color='red')
