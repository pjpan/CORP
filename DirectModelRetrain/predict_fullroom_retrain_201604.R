...
rawdata <- fread(input ="./directmodel0412_0418.txt", header = T, na.strings = "NULL", sep="\t", stringsAsFactors = F)
...
HTL_ALL%>%filter(avldaysbefhol==2)%>%head(20)
...
HTL_ALL%>%count(avldaysbefhol, isfullroom)%>%mutate(pct =n/sum(n))
...
train <- createDataPartition(HTL_lowstar$isfullroom, p = 0.7, list = F)
...
save(direct.ntrees, direct.gbmmodel , file = 'ProdScripts/ReservedRoomv7.0.RData')
...
load("ProdScripts/ReservedRoomv6.0.RData")
...
# 0.8116309    v6.5 +facotr(room_noroomsumorder)+factor(province) -1 0 1 2 -intense
...
# 对比不同模型的结果：
p <- ggplot(data = roc.result.v7.0)+geom_line(aes(x= recall , y=  accuracy,fill = c("allfeatures")),color = 'red')
...
# 把模型结果存储下来；
save(direct.gbmmodel, direct.ntrees, file = './ReservedRoomv7.0.RData')
