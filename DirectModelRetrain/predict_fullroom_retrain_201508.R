...
HTL_ALL <- cbind(rawbetalog_HTL, isfullroom = ifelse(rawdata$cancelreason=='NOROOM',1,0), vdate = rawdata$d.1)
...
HTL_test <- filter(HTL_ALL, star<=3 ,ordprior>=0, vdate == '2015-08-31')
HTL_lowstar <- select(filter(HTL_ALL ,star<=3 ,ordprior>=0, vdate<'2015-08-31')
                      ,room_sumorder
                      ,room_noroomsumorder
                      ,htl_noroomsumorder
                      ,htl_sumorder
...
rfmodel <- randomForest(as.formula(rf.formula)                   
                        ,data=HTL_lowstar
                        ,ntree=500            # 决策树的数量
                        ,keep.forest=F        # 是否保留每棵树的结果
                        ,proximity=F          # 是否计算变量和变量之间的关系，T下会慢
                        ,importance=T         # 
...
nrow <- NULL
roc <- NULL
seq2 <- seq(0.05,0.4,by=0.05)
for(i in 1:length(seq2))
{
train <- rbinom(dim(HTL_lowstar)[[1]],size=1 ,prob = seq2[i])==1
HTL_lowstar_train <- HTL_lowstar[train,]
nrow[i] <- nrow(HTL_lowstar[train,])
...
# predict testdata
HTL_test$pred <- predict(direct.gbmmodel, newdata = HTL_test, type="response",n.trees = direct.ntrees)
roc[i] <- gbm.roc.area(HTL_test$isfullroom, HTL_test$pred)
}
ggplot(data = HTL_test)+geom_density(aes(x= pred , color=factor(isfullroom)))+geom_density(data = HTL_test, aes(x= pred ))
ggplot()+geom_density(data = HTL_test, aes(x= pred ))
summary(pred.gbm)
#查看分布图
...
View(roc.result)
p <- ggplot(data = roc.result)+geom_line(aes(x= sensitivity , y=  specificity))
...
# 对误判的订单进行聚类；
names(HTL_lowstar_test)
library(cluster)
HTL_lowstar_sample <- HTL_lowstar_test%>%filter(pred<=0.05)%>%select()
clusters <- kmeans(HTL_lowstar_sample , centers = 5)
HTL_lowstar_sample$labels <- clusters$cluster

#画出每个类别的结果
ggplot(data = HTL_lowstar_sample)+geom_point(aes(x= pred, y =　pct_htlnoroomrate , colour = factor(labels)))+facet_grid(.~isfullroom)
HTL_lowstar_sample%>%count(isfullroom, labels)%>%mutate(pct = n/sum(n))
HTL_lowstar_sample%>%count(labels, isfullroom)%>%mutate(pct = n/sum(n))
